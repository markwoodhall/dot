...
plugins=(archlinux git postgres tmux lein npm zsh-autosuggestions mercurial aws docker docker-compose terraform zsh-syntax-highlighting)
             ...
             {:left 
              ...
              [blocks.separator
               ...
               (when (= (hostname) "thinking")
                 {:load (partial (. blocks.free-disk-space :load) "/" "data")
                  :draw (partial (. blocks.free-disk-space :draw) "/" "data" "/ ")})
               blocks.separator
               blocks.pacman]})
...
#+begin_src edn :tangle ~/.config/clj-kondo/config.edn :mkdirp yes
...
** shadow
#+begin_src edn :tangle ~/.shadow-cljs/config.edn :mkdirp yes
...
#+begin_src edn :tangle ~/.config/feedbb.edn
  {:feeddir "/home/markwoodhall/feed"
   :feeds [{:url "https://echasnovski.com/blog.xml" :type :rss :slug "echasnovski" :tags [:blog :neovim :vim]}
           {:url "https://martintrojer.github.io/index.xml" :type :rss :slug "martintrojer" :fetch-each? true :tags [:blog :software :vim :ai]}
           {:url "https://feeds.feedburner.com/ThePragmaticEngineer" :type :rss :slug "thepragmaticengineer" :fetch-each? true :tags [:blog :software :engineering]}
           {:url "https://planet.emacslife.com/atom.xml" :type :atom :slug "planetemacs" :tags [:emacs :aggregation]} 
           {:url "https://hgrsd.nl/sitemap.xml" :type :sitemap :slug "hgrsd.nl" :fetch-each? true :sitemap-query "/blog/[^/]+/?$" :non-parseable-date? true :tags [:blog :emacs :vim]}
           {:url "https://lpar.ath0.com/sitemap.xml" :type :sitemap :slug "lpar" :fetch-each? true :sitemap-query "/posts/[^/].*" :tags [:blog :vim]}
           {:url "https://www-gem.codeberg.page/feed.xml" :type :atom :slug "wwwgem" :fetch-each? true :tags [:blog :neovim]}
           {:url "https://mastodon.social/tags/emacs.rss" :type :rss :slug "mastodon-emacs" :dir "mastodon" :org-header {:no-title? true} :tags [:mastodon :emacs]}
           {:url "https://mastodon.social/tags/vim.rss" :type :rss :slug "mastodon-vim" :dir "mastodon" :org-header {:no-title? true} :tags [:mastodon :vim]}
           {:url "https://mastodon.social/tags/neovim.rss" :type :rss :slug "mastodon-neovim" :dir "mastodon" :org-header {:no-title? true} :tags [:mastodon :vim :neovim]}]}
#+end_src
